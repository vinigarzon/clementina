-- =====================================================================
-- Finca La Clementina · Migración 0008
-- Aplica el esquema comercial completo (originalmente diseñado en 0003)
-- de forma idempotente, compatible con lo que ya existe en producción
-- (en particular: la tabla public.leads y el enum public.lead_status
-- creados por la migración 0007 cuando solo necesitábamos guardar
-- solicitudes desde el formulario público).
--
-- Crea (si no existen):
--   - Enums: event_status, date_hold_status, quote_status, contract_status
--            (lead_status ya existe desde 0007 y se respeta).
--   - Tablas: clients, spaces, events, date_holds, catalog_categories,
--             catalog_items, vendors, quotes, quote_lines, contracts,
--             payment_schedules, payments, costs.
--   - Triggers updated_at, RLS y policies para todos los módulos.
--   - Foreign keys en public.leads (client_id, event_id) que faltaban.
--
-- Aplicar pegando este archivo entero en Supabase → SQL Editor.
-- =====================================================================
...
-- lead_status ya existe (creado en 0007). No se intenta recrear.
...
-- ---------- LEADS (ya creada en 0007; aquí solo añadimos las FKs) ----------
-- client_id → clients(id)
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'leads_client_id_fkey'
...
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'leads_event_id_fkey'
      and conrelid = 'public.leads'::regclass
  ) then
    alter table public.leads
      add constraint leads_event_id_fkey
...
-- Leads (mantiene la policy leads_public_insert creada en 0007)
...
-- Refresca el cache de PostgREST para que el admin vea las tablas nuevas.
